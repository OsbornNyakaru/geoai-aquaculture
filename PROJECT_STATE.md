...
3. **Current next action: `Run all` → iter35 (CAPACITY-NEUTRAL τ scan + the REAL single-τ seed-avg) on Colab.**
   iter34 SETTLED that adding a SECOND channel HURTS (pondband −0.0061, vhsq −0.0178, vvperm −0.0153;
   ricegate not yet uploaded). Clean mechanism: every arm's OOF rose but every LB fell → extra Linear
   params overfit the adv-AUC-0.89 shift. The **25-channel permanence model is a CAPACITY SWEET SPOT**;
   single-channel-ADDITION lane is closing. iter35 does the only no-capacity-tax lever: (1) scan the
   SINGLE permanence τ ∈ {−22,−20.5,−20} (each stays 25 ch) — is −21 the best cut? (2) build the REAL
   single-τ 5-seed avg `champion_perm_seedavg5_st` (the 0.8969 upload was the STALE 4-τ file; expect
   ≥0.90 given s42 0.9065 + s29 0.9007) = the permanence FINALIST. Also pending: upload leftover
   `submission_c_perm_ricegate.csv` (last add-lane test, low prior). **Then deadline-bound:** DESIGNATE
   finalists (leading pair: `champion_perm_seedavg5_st` + `champion_archblend4` 0.899643 — different
   error profiles) + Phase-Two writeup (35% of top-5 rubric, still unbuilt).
