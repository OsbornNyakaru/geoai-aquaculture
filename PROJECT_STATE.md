...
3. **Current next action: `Run all` → iter34 (the SECOND FEATURE, one at a time) on Colab, up to 2 uploads.**
   iter33 SETTLED that permanence does NOT stack in an ensemble (`champion_perm_archblend4` = 0.892939,
   −0.0067 vs base archblend4, −0.0136 vs the single 0.9065): the two +0.010 lifts are SUBSTITUTES.
   So permanence is a SINGLE-model lever. iter34 grows the single permanence champion by ONE new
   nonlinear-in-the-mean channel at a time (UPDATE_15 menu): (A) `vv_permanence`, (B) `pond_band`,
   (C) `vh_sq`→Var, (D) `rice_gate` SARxoptical. seed 42 directional; each paired vs a same-run
   c_perm_single baseline (must reproduce 0.906492). ≥ +0.006 → carry to iter35 seed-confirm.
   **FREE PENDING:** also upload `submission_champion_perm_seedavg5.csv` (single-τ, 5-seed avg) from
   the iter33 run — the honest seed-robust permanence number / natural permanence FINALIST (~0.897-0.900
   expected; confirms 0.9065 was seed-42 luck). **Then deadline-bound:** DESIGNATE finalists (leading
   pair: `c_perm_single`/its seed-avg + `champion_archblend4`) + Phase-Two writeup.
