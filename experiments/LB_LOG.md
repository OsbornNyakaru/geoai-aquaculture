...
| 49 | 2026-08-14 | **JTT (Just Train Twice, Liu et al. ICML 2021) — the last live candidate for the F1 gap, and the only mechanism round 23 found that PROVABLY reorders.** Round 23 closed every operator-level lane on arithmetic (threshold +0.0004; calibrator family direction-reversed; pooling operator 4–13 rows twice; and EVERY pointwise loss order-invariant at the population optimum — focal/ASL/LDAM/PolyLoss/label-smoothing all minimize to T(eta(x)) for one fixed monotone T, arXiv:2011.09172 Thm 3/5/11 + Lemma 14). **Both external Gemini Deep Research reports ranked Asymmetric Loss #1; that is refuted by the theorem and we did not run it.** JTT escapes the theorem because its weight is x-dependent AND class-asymmetric, so eta_eff = eta·w1/(eta·w1+(1−eta)·w0) is not monotone in eta alone. Stage-1 error set read from a banked OUT-OF-FOLD bundle rather than retrained, which removes JTT's worst knob (the early-stopping epoch T) since an OOF prediction cannot memorize. Error set size 38 of 1817 (24 FN + 14 FP); the 24 missed positives are CONFIDENTLY missed (median OOF 0.170, ten below 0.10, ZERO in [0.45,0.50)). lambda_up pre-committed to the parameter-free `balance` rule. | submission_jtt_lam5_s42.csv | 0.5864 | **0.908387851** [AUC **0.944824076**, F1 **0.884097035**] | ➖ **INCONCLUSIVE — the pre-registered ±0.006 branch. LANE CLOSED, and the mechanism is refuted by an exact integer identity.** vs champion 0.907368983 (stated explicitly, since no slot was spent on the control): composite **+0.001019**, AUC **−0.001018**, F1 **+0.002377**. **THE F1 CELL INVERTS UNIQUELY to TP=164, PP+P=371 against the champion's TP=164, PP+P=372 — TP IS IDENTICAL AND PP FELL BY ONE, so the entire F1 gain is ONE FALSE POSITIVE REMOVED AND ZERO TRUE POSITIVES RECOVERED.** JTT exists to recover confidently-missed positives; on the public slice it recovered none. **It also forfeits the column we were winning:** AUC 0.944824076 falls BELOW the leader's 0.944897, where the champion's 0.945841814 beats it — 22 concordant pairs traded for one FP, and on the private 721 rows ranking is the stabler half. **THE OFFLINE MECHANISM FIRED BUT DID NOT HELP:** rho vs control 0.9840 (λ=5) and 0.9427 (λ=46.8), far below the pre-registered 0.999 degeneracy line, so this is the FIRST PROVABLE REORDERING this project has produced — yet out-of-fold error-set recovery was 7/24 against a **control baseline of 6** (net **+1**), and the FP column was identical (3/14) in all three runs. ARM A (λ=46.8) was closed OFFLINE with no slot: OOF combined −0.0166 and net −1 on the target rows, exactly the pre-registered memorization risk (half the gradient mass on 38 rows in a 71k-param model). 🔬 **A METHODOLOGICAL RESULT WORTH MORE THAN THE ARM:** OOF composite said +0.00091 and was uninformative as always, but the **mechanism-specific, control-baselined** instrument said "+1 of 24 ≈ nothing" and **correctly predicted 0 TP on the LB**. Hypothesis for the report (n=1): our offline instruments fail when they measure AGGREGATE performance and can work when they measure THE SPECIFIC MECHANISM against a control — every retired instrument here (OOF, adv-AUC, ATC-F1) is of the aggregate kind. ⛔ **Two self-corrections this iteration:** `jtt_gate.py` v1 mislabelled the recovery count "in-sample" (wrong — row i is never upweighted in the model that predicts it) AND omitted the control baseline, without which "7 of 24" reads as a success; and `run_current.sh` pointed at gitignored `submissions/preds/` bundles, so it died on a fresh Colab clone with a FileNotFoundError four minutes in (now regenerates its dependencies + preflights in seconds). **FINALISTS UNCHANGED** — single-seed, inside noise, below our best-ever 0.910837, forfeits the AUC lead. **ROUND 23 IS FINISHED: the confidently-missed positives are not recoverable from the source distribution, now measured rather than argued.** |
| 47 | 2026-08-14 | **THE PRESTO LANE, REOPENED AND ACTUALLY SUBMITTED — 2 arms, 1 variable (frozen vs fine-tuned).** iter17 killed this lane for ZERO submissions on adv-AUC (retired round 18), ATC-F1 (retired iter25/26) and OOF (blind by standing rule) — all three withdrawn by this project since, so the kill rested entirely on retracted evidence and Presto has never been on the leaderboard. ARM A = frozen 404,160-param encoder + 129-param logistic head (the iter17 arm, now routed through `calibrate_legal` and uploaded). ARM B = all 404,160 params unfrozen, 8 fixed epochs, no early stopping and no LR search (early stopping needs a selection signal and every offline signal we have is retired or blind). Pre-registered read committed at `8998042` BEFORE any number existed. `fetch_presto.py` fixed en route: `REV` said `"main"` under a comment claiming byte-identical reproducibility — now pinned to SHA `11e207a6` + SHA-256 checkpoint gate. | submission_presto_frozen.csv / submission_presto_finetune.csv | 0.5699 / 0.5670 | **0.805184519 / 0.811475341** | ❌❌ **THE LANE IS CLOSED ON A MEASUREMENT — −0.102 / −0.096 vs champion — which was the entire point of reopening it.** frozen AUC 0.827675196 F1 0.790190735 (cell **145/367, UNIQUE inversion**); finetune AUC 0.836907531 F1 0.794520547 (cell **145/365**). ARM A reads the pre-registered "0.80–0.895 → transfers but loses, record and close" branch, clearing "does not transfer at all" by 0.005. Note WHERE it loses: AUC −0.118 vs champion but F1 only −0.092, i.e. a general-purpose encoder is worse at RANKING this task than a purpose-built shifted-domain model — the opposite of the usual foundation-model story. ⚠️ **ARM B − ARM A = +0.006290821: OUR PRE-REGISTERED PREDICTION MISSED IN SIGN.** We committed in writing at `8998042`, before any number existed, that the fine-tuned arm would LOSE by ≥0.006 under the "added capacity fitted to shifted rows hurts" law; fine-tuning 404,160 params onto the shifted rows WON, firing the `>= +0.006` "CONTRADICTS our capacity law" branch. **But it fires by 0.00029 = 1/50th of the ±0.015 binomial band**, so all three of these are true at once and we assert all three: the prediction missed in sign; we CANNOT claim the law is refuted; and we CANNOT claim it held out-of-family — the writeup deliverable this run was meant to produce ("the law extends to a pretrained model") **did not happen and is not being written up as though it did.** Decomposition: 59% ranking / 41% cut (ΔAUC +0.009232×0.4, ΔF1 +0.004330×0.6). The F1 cell is exact and clean: **TP = 145 in BOTH arms while PP+P falls 367→365, so fine-tuning deleted exactly two predicted positives and BOTH were FALSE POSITIVES, zero TP lost** — the same identical-TP denominator-only signature as iter44's dpa arm. Five seeds would resolve whether either half is real; two days out and 0.10 below champion, we decline to spend them. **OOF was blind again but ordered the arms correctly** (0.9673<0.9723 matched 0.8052<0.8114) — one concordant pair, not a rehabilitation; the level gap is ~0.16 in both. **DECORRELATION BOUGHT NOTHING, as pre-registered:** ρ 0.816/0.841 vs champion made these the most decorrelated artifacts we ever built and they scored 0.10 low, so a weak decorrelated member has now lost three times (ROCKET −0.009, GBDT −0.0155, Presto −0.10). 🔑 **THE VALUABLE BYPRODUCT — P IS PINNED THREE WAYS AND SAYS WE UNDER-PREDICT.** frozen's unique inversion plus each artifact's pos-rate × 309/1030 gives P = **191.9 / 190.9 / 189.8** from two unrelated model families, tightening `190 ± 7` to **190–192**. At P=191 the champion's public cell is PP=181, TP=164, **precision 0.906 vs recall 0.859**, implied public prevalence **0.618 vs our operating 0.583** — precision exceeding recall by 0.047 is the signature of a cut placed **too HIGH**, ~10 too few positives on public. ⛔ **DIAGNOSIS ONLY — every number there is LB-inverted, and `compliance_mode: legal` forbids threshold tuning outright.** Recorded as the sharpest OPEN question, not a lever: is there a **train-only** criterion reaching the same conclusion, plus a legal mechanism to act on it? It is in tension with our own train-only δ̂ = F*/2 = 0.479–0.485, which agrees in DIRECTION but sizes the effect at ~0.02 not ~0.035. **THE ENTIRE GAP TO FIRST PLACE IS THE F1 TERM:** the leader's AUC 0.944897 is already beaten by our 0.945841814, so their composite 0.929–0.936 implies F1 **0.918–0.930 vs our 0.881720** = +0.037 to +0.048 F1 = +0.022 to +0.029 composite. → **round 23 is about F1 and nothing else.** Finalists unchanged (ARM A ≠ ≥0.913). |
| ~~47~~ | ~~staged~~ | *(pre-upload staging notes, superseded by the result row above)* | — | — | — | ⏳ **RAN LOCALLY, BOTH ARMS VALID, AWAITING LB.** Void check PASSES: ARM B train BCE fell 0.31768 → 0.09933 on fold 0 and landed 0.087–0.101 across all five folds, so the fine-tune trained and its result will measure fine-tuning rather than a broken optimizer. Local OOF (BLIND, recorded only for the paste-back list): frozen combined 0.9673 / AUC 0.9909 / f1 0.9515; fine-tuned 0.9723 / AUC 0.9917 / f1 0.9593. **Platt slope 1.299 → 1.021** is the clean free finding: the frozen linear probe is systematically under-confident exactly as a probe on a frozen general-purpose representation should be, while the end-to-end model is nearly natively calibrated — descriptive only, since Platt annihilation means `calibrate_legal` refits the slope regardless. **TWO SLOTS WERE JUSTIFIED BY MEASUREMENT, not assumed:** ρ(A,B) = 0.9205 (71/1030 hard-label disagreement) so ARM B is not a re-upload of ARM A, and ρ(A,champion) = **0.8157** / ρ(B,champion) = **0.8405** (135 and 124 rows disagreeing) make these **the most decorrelated artifacts this project has ever produced** — for scale, seed replicates sit at ρ≈0.95 and the iter46 pooling arms at ρ≈0.99998. ⚠️ That decorrelation justifies the second UPLOAD SLOT and nothing else: a weaker decorrelated member has lost twice here (ROCKET −0.009, GBDT −0.0155) and Presto's level is still unknown. **THIRD INDEPENDENT POS-RATE CHECK PASSES:** 0.5699/0.5670 vs champion 0.5670 from a model sharing no architecture, code path or training corpus — joining graph propagation (0.591–0.599) and the graph estimate (0.587) in the band we already operate in. Diagnosis only; its value is that the operating point survived a check it could have failed. Honest prior stated before the run: this project's own most reliable law ("added capacity fitted to our 1,817 shifted rows hurts") predicts ARM B LOSES to ARM A, and frozen Presto's 129 fitted params never tested that law. **Finalists {champion_dualpolmix10_regimematch 0.907368983, champion_archblend4 0.899643} unchanged unless ARM A ≥ 0.913**, the only branch that reopens the lock. Also this round, zero submissions: `tools/feature_span_gate.py`, a free VETO instrument — VH−VV is EXACTLY linear in two supplied columns so a model receiving both can already represent it, which is the mechanical form of iter43's three VH−VV nulls. **The gate's v1 FAILED ITS OWN CONTROL** (used `median(VH−VV)`, R²=0.6206, because a median is nonlinear in the raw values) and was caught before publication; the honest single-month control returns 1.0000/1.0000. LASCI (span R² 0.7526, window ρ 0.8992, univ AUC 0.8891) is the only candidate clearing both gates — recorded, NOT built, two days out. Round-22 also corrected two of OUR premises: the Ottinger canonical feature is **VH alone, pixel-wise temporal median** (the dual-pol ratio is not in that pipeline at all; Ullmann 2022 measures polarimetric derivatives at **+0.1%** for water), so the VH−VV null was the literature's prediction and we had mis-cited our own motivation; and the agent's #1 recommendation (contiguous 4–6 month crop augmentation) **is already implemented** in `_mask_views` via `sample_window(wd, ...)` — the second time a research round has proposed the masked replica we already had. |
...
reproduces `0.6·F1 + 0.4·AUC` exactly.

Against `champion_dualpolmix10_regimematch` (0.907368983), and **saying so explicitly as the
pre-registered read requires, because we did not spend a slot on the control**:

| | Δ | ×weight |
|---|---|---|
| composite | **+0.001018868** | — |
| AUC | **−0.001017738** | −0.000407 |
| F1 | **+0.002376605** | +0.001426 |

**Pre-registered branch: `within ±0.006` ⇒ inconclusive at our noise floor. Report as such; do NOT
call it a win. The lane closes.** That is the branch, and we are taking it. The comparison is in fact
*less* favourable than the raw delta suggests: this is a **single-seed** arm against a **10-seed**
artifact, and single seeds carry sd 0.019 here and have produced three separate mirages
(perm/vhsq/mean_min, all 0.91-something → 0.8995).

### 🔑 THE F1 CELL INVERTS UNIQUELY, AND IT REFUTES JTT'S THESIS OUTRIGHT

`0.884097035` has exactly one solution over D ∈ [250, 500]: **TP = 164, PP + P = 371.**
The champion's cell is **TP = 164, PP + P = 372.**

> **TP is IDENTICAL at 164. `PP` fell by exactly ONE. The entire F1 "gain" is one false positive
> removed, and ZERO true positives recovered.**

JTT exists to recover confidently-missed positives. On the public slice it recovered **none**. This
is not a marginal result to be argued over — it is an exact integer identity, and it agrees with what
the offline gate already said (net **+1** of 24 target rows recovered out-of-fold versus the control
baseline of 6, i.e. nothing).

**And it paid for that one false positive with the column we were winning.** AUC −0.001018 ≈ 22
concordant pairs, which drops the arm to **0.944824076 against the leader's 0.944897 — below it.**
Our champion's 0.945841814 is one of only two artifacts we have ever produced that out-ranks the
leader. JTT trades that away to delete a single FP. On the private 721 rows, ranking is the more
stable half and a one-row cut change is pure luck.

### 🔬 A METHODOLOGICAL RESULT WORTH MORE THAN THE ARM: a targeted offline instrument PREDICTED the LB

This project's standing rule is that OOF is blind, and it held again — OOF composite said **+0.00091**
for this arm and carried no information. But the **mechanism-specific, control-baselined** instrument
did predict the outcome:

| instrument | said | LB said | predictive? |
|---|---|---|---|
| OOF composite | +0.00091 | +0.0010 composite, but 0 TP | no — right number, wrong reason |
| error-set recovery **vs control** | **+1 of 24** ≈ nothing | **0 TP gained** | **yes** |

The difference is the control baseline. "7 of 24 recovered" looks like success; "7 versus the control's
6" is a null, and the null was correct. **Hypothesis worth carrying into the report (n=1, stated as a
hypothesis, not a law): offline instruments in this project fail when they measure AGGREGATE
performance and can work when they measure THE SPECIFIC MECHANISM the arm claims, against a control
that isolates it.** Every retired instrument here — OOF, adv-AUC, ATC-F1 — is of the aggregate kind.

### Status

**LANE CLOSED. FINALISTS UNCHANGED**, exactly as pre-registered: a single-seed arm cannot replace a
10-seed measured artifact, and this one is inside noise, below our best-ever 0.910837, and forfeits
the AUC lead. `{champion_dualpolmix10_regimematch 0.907368983, champion_archblend4 0.899643}` stand.

**Round 23 is finished, and every branch closed on a measurement:**

| lane | closed by |
|---|---|
| moving the threshold | +0.0004 F1, ~60× below noise |
| calibrator family | direction reversed, 15 down / 0 up |
| pooling operator | 4–13 rows of 1030, twice |
| any pointwise loss | order-invariant at the population optimum (arXiv:2011.09172) |
| an x-dependent reweighting that provably reorders | **reorders — and recovers 0 TP on the LB** |

> **The conclusion, now measured on the leaderboard rather than argued: the confidently-missed
> positives are not recoverable from the source distribution.** The last mechanism standing did
> exactly what the theory promised — it genuinely reordered, ρ 0.984 — and still could not move a
> single true positive across the cut. The 4–6 month test window does not contain the evidence.
> That is a property of the data the organizers built, not a deficiency of the model, and it is the
> finding this report should lead with.
...
|---|---|---|---|---|
| `presto_frozen` | **0.805184519** | 0.827675196 | 0.790190735 | **145 / 367** (unique inversion) |
| `presto_finetune` | **0.811475341** | 0.836907531 | 0.794520547 | **145 / 365** |
| champion (for scale) | 0.907368983 | 0.945841814 | 0.881720430 | 164 / 372 |

Both composites reproduce `0.6·F1 + 0.4·AUC` to 4e-10. The lane is now closed on a **measurement**,
which was the entire point of reopening it: −0.102 and −0.096 against the champion.

**ARM A reads into the pre-registered "0.80 – 0.895 → the lane transfers but loses. Expected outcome.
Record and close."** It cleared the "does not transfer at all" branch by 0.005 — Presto barely
transfers. Note *where* it loses: AUC −0.118 against our champion but F1 only −0.092. A general-purpose
pixel-timeseries encoder is far worse at **ranking** this task than a purpose-built shifted-domain
model, which is the opposite of the usual foundation-model story and worth one line in the report.

### ⚠️ ARM B − ARM A = **+0.006290821**. The pre-registered prediction was wrong in sign.

This project committed, in writing at `8998042` and before any number existed: *"the honest prior is
that the fine-tuned arm LOSES to the frozen arm… Say so now, before the numbers, so we cannot claim
afterwards that we expected either."* The law behind it — "added capacity fitted to our 1,817 shifted
rows hurts" — is one of the most reliable findings in this ledger. **Fine-tuning 404,160 parameters
onto the shifted rows WON.** The `>= +0.006` branch fires, whose text reads: *"fine-tuning HELPS,
which would CONTRADICT our own capacity law… It would mean the law is about RANDOM initialization, not
capacity per se. Do not act on one seed; expand to five and re-read."*

**Now the discipline, because the branch fires by 0.00029.** That margin is **1/50th of the ±0.015
binomial noise band** on 309 rows. So the honest three-part statement is:

1. Our pre-registered prediction **missed, in sign**. Logged as a miss, as iter42's was.
2. We **cannot** claim the capacity law was refuted — a 0.0003 clearance of a bar is not a refutation
   of anything, and the pre-registration itself said not to act on one seed.
3. We **also cannot** claim the law held out-of-family. It did not. The valuable thing this run was
   supposed to produce for the code review — "the law extends to a pretrained model" — **did not
   happen, and we are not writing it up as though it did.**

The mechanism, decomposed: **59% of ARM B's gain is ranking, 41% is the cut.** ΔAUC +0.009232 (×0.4 =
+0.003693) and ΔF1 +0.004330 (×0.6 = +0.002598). And the F1 cell is exact and clean: **TP is 145 in
BOTH arms while `PP+P` falls 367 → 365, so fine-tuning removed exactly two predicted positives and
BOTH were FALSE POSITIVES — zero true positives lost.** The identical-TP, denominator-only move is the
same signature as iter44's `dpa` arm. With five seeds we would know whether either half is real; with
two days left and the lane 0.10 below the champion, **we are not spending them.** Lane closed.

**OOF was blind again, but it did order the two arms correctly** (0.9673 < 0.9723 matched 0.8052 <
0.8114). One concordant pair is not a rehabilitation of OOF; the level gap is ~0.16 in both arms.

**Decorrelation bought nothing, exactly as pre-registered.** These were the most decorrelated artifacts
the project has produced (ρ 0.816 / 0.841 vs champion) and they scored 0.10 low. The standing rule
holds unchanged: **decorrelation is not value**, and a weak decorrelated member has now lost three
times here (ROCKET −0.009, GBDT −0.0155, Presto −0.10 standalone).

### 🔑 The genuinely valuable byproduct: P is now pinned three ways, and it says we UNDER-predict

`presto_frozen` inverts **uniquely** to TP=145, PP+P=367 — no other (TP, D) with D ∈ [250,500]
truncates to 0.790190735 at nine decimals. Combining each artifact's known full-test positive rate
with the assumption that the public 309 is a uniform subsample of the 1030:

| artifact | pos-rate | implied PP on public | ⇒ P = D − PP |
|---|---|---|---|
| `champion_dualpolmix10_regimematch` | 0.5830 | 180.1 | **191.9** |
| `presto_frozen` | 0.5699 | 176.1 | **190.9** |
| `presto_finetune` | 0.5670 | 175.2 | **189.8** |

Three artifacts from two unrelated model families agree on **P ≈ 190–192**, tightening the previous
...
the operating point* — exists for exactly this temptation, and `compliance_mode: legal` forbids
threshold tuning outright. This is recorded as **the sharpest open question in the project**, not as a
lever: *is there a **train-only** criterion that independently reaches the same conclusion, and a
legal mechanism that acts on it?* Note the tension it creates with our own train-only instrument:
iter44/45's `δ̂ = F*/2` landed at **0.479–0.485**, which agrees in DIRECTION (optimum below 0.5) but
says the effect is ~0.02, not the ~0.035 the LB arithmetic implies. That tension is the seed of the
next research round.

### Where the whole remaining gap to the leader lives

The leader sits at ~0.929–0.936 with AUC **0.944897**, which our champion already beats
(0.945841814). Solving for their F1: **0.918 / 0.923 / 0.930** against our 0.881720. **The entire gap
to first place is the F1 term** — between +0.037 and +0.048 of F1, i.e. +0.022 to +0.029 composite.
Everything else is settled. → this is the sole subject of round 23.
